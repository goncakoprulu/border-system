    ...
    return <Panel>
      ...
      {data.students.length === 0 ? <Empty title="Bu sınıfta aktif öğrenci yok" detail="Ders oturumu tamamlanabilir; yoklama listesine yalnızca ders tarihinde aktif kaydı olan öğrenciler alınır."/> : <div className="divide-y">{data.students.map((student) => {
        ...
        return <div key={student.studentId} className="p-4 sm:p-5">
          <div className="flex items-center justify-between gap-3"><p className="font-medium">{student.studentName}</p><Button type="button" size="sm" variant="ghost" onClick={() => setOpenNotes((currentSet) => { const next = new Set(currentSet); if (next.has(student.studentId)) next.delete(student.studentId); else next.add(student.studentId); return next; })}><StickyNote />{note ? "Notu düzenle" : "Not"}</Button></div>
