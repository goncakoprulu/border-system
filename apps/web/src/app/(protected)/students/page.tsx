...
export default function StudentsPage() {
  ...
  const [assigningStudent, setAssigningStudent] = useState<{ id: string; name: string }>();
  const canManage = user?.roles.some((role) => ["Admin", "Management", "Reception"].includes(role)) ?? false;
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    {canManage && <StudentClassAssignmentDialog student={assigningStudent} open={Boolean(assigningStudent)} onOpenChange={(open) => { if (!open) setAssigningStudent(undefined); }} />}
  </div>;
}
